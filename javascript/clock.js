...
var x = window.matchMedia('@media all and (max-width: 719px)');
if(x.matches) {
	...
	if (12 <= placeCircle && placeCircle < 18) {
		if (circle) {
			document.getElementById("circle").style.left="30px";
		}
	}
} else {
	if (7 <= placeCircle && placeCircle < 12) {
	...
	if (18 <= placeCircle && placeCircle < 20) {
		if (circle) {
			document.getElementById("circle").style.left="500px";
		}
	}
	if (18 <= placeCircle && placeCircle > 7) {
		if (circle) {
			document.getElementById("circle").style.left="800px";
		}
	}
} 
